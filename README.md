...
Every service is in a Docker container and they are all managed using Docker Compose.
...
- **Data Flattening**
  - Because the consumed API has semi-structured data instead of structured, to store this data into a relational database, I flattened it using Pydantic. This approach was made modeling a data validation class that checks (and coerces type conversions for each field of a record), and then the [dict](https://github.com/Victor-Buendia/fn-challenge/blob/44e11d1cff3c3c64d9b345b57a04f31f85470620/scripts/validation/schemas.py#L64) function makes the record a single depth level.
  - Other approaches could have been taken, like using [JSONB](https://hashrocket.com/blog/posts/dealing-with-nested-json-objects-in-postgresql) data type to save the nested fields.
- **Table management with ORM**
  - Object Relational Mapping (ORM) was used in SQL Alchemy library because it maintains a consistency of the script using Python to manipulate data in the database. Using ORM also makes the code more  standardized, because we use methods of the library; safer, because it avoids SQL injections in data to be inserted; and tool-agnostic, because it creates an abstract layer regarding the connection to PostGres, allowing a database change in the future.
- **Data quality**
  - Since data being processed has a reasonable size, Pydantic shows as a viable way of ensuring a level of data quality. It is used to coerce data to the correct data type of each field and to raise an error if a data doesn't follow a business rule or can't be type casted.
...
The below requirements are necessary for running the project the way it was intended to and their versions are also showed, meaning that using these versions or above is recommended for a correct execution.
...
1. *env-check:* Asserting if a `.env` file exists. If not, creates one following the already presented model.
2. *basic-services:* Starts the PostGres and PgAdmin4 in detached mode. After that, starts the Python pipeline, triggered by `scripts/main.py` execution.
...
│       │   └── views
│                └── user_events_view.sql         # Creates a simple view of the data in the database
...
│   ├── environment.py                            # Class that deals with environment variables
│   ├── main.py                                   # Triggers the pipeline execution
│   ├── models
...
- Object Relational Mapping (ORM) was used in this context assuming a high synergy between the data and product team. This is due to the modeling created for the data being ingested. It's important to ensure that the schema of the incoming data is already known by the data engineering team.
